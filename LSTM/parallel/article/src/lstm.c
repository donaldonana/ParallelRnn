...
float lstm_validation(lstm_rnn* lstm, Data* data)
{
  ...
  for (int i = start; i <= end; i++)
  {
    ...
    // Compute loss
    Loss = Loss + loss_entropy(data->Y[i], lstm->probs, lstm->Y);
    // Compute accuracy
    acc = accuracy(acc , data->Y[i], lstm->probs, lstm->Y);
    n = n + 1 ;
  }
...
float lstm_test(lstm_rnn* lstm, Data* data, int execution, int thread, FILE* ft)
{
  ...
  for (int i = start; i <= end; i++)
  {
    ...
    fprintf(ft,"%d,%d,%d\n", thread, ArgMax(data->Y[i], data->ycol) , ArgMax(lstm->probs, data->ycol ));
    // Compute accuracy
